...
def lengths(n,approx,image):
    for i in range(n):
        leng = math.hypot(approx[i-1][0][0]-approx[i][0][0],approx[i-1][0][1]-approx[i][0][1])
        leng = str(round(leng))
        leng_pos = (int((approx[i-1][0][0]+approx[i][0][0])/2),int((approx[i-1][0][1]+approx[i][0][1])/2))
        cv2.putText(image, leng, leng_pos, cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(0, 0, 0), thickness=4)
        cv2.putText(image, leng, leng_pos, cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(255, 255, 255), thickness=2)
