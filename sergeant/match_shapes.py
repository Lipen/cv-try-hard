	...
	while True:
		...
		for contour in contours:
			if cv2.contourArea(contour) < 500:
				continue
			match_coefs.insert(i,cv2.matchShapes(contour, base_contour, 1, 0))
			cv2.drawContours(image, [contour], -1, (0, 255, 0), 2)
			# asid = match_coefs[i]
			if match_coefs[i] < match_min:
				match_min = match_coefs[i]
				i_min = i
			i += 1
			
		text = '{:.3f}'.format(match_min)
		if cv2.contourArea(contours[i_min]) > 400:
			cv2.drawContours(image, [contours[i_min]], -1, (0, 0, 255), 2)

		M = cv2.moments(contours[i_min])
